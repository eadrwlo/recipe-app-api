...
class PrivateRecipeAPITests(TestCase):
    ...
    def test_recipe_list_limited_to_user(self):
        """Test list of recipers are limited to auth user."""
        other_user = create_user(email='userother@example.com',
                                 password='testpass123')
        create_recipe(user=self.user)
        ...
        list_of_values_recipe = [i for i in recipes.values()]
        od = OrderedDict([(k, v) for i in list_of_values_recipe for
                          k, v in i.items()])
        del od['user_id']
        del od['description']
        # print("od=", od)
    ...
    def test_create_tag_on_update_recipe(self):
        ...
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        new_tag = Tag.objects.get(user__exact=self.user, name='Lunch')
    ...
    def test_create_ingredient_on_update_recipe(self):
        ...
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        ingredient = Ingredient.objects.get(user__exact=self.user,
                                            name='Pepper')
        self.assertIn(ingredient, recipe.ingredients.all())

    def test_update_recipe_assign_ingredient(self):
        """Test assigning tag when updating a recipe."""
        ingredient_pepper = Ingredient.objects.create(user=self.user,
                                                      name='Pepper')
        recipe = create_recipe(user=self.user)
    ...
    def test_filter_by_tags(self):
        ...
        r1 = create_recipe(user=self.user, title='First recipe')
        r2 = create_recipe(user=self.user, title='Second recipe')
        tag1 = Tag.objects.create(user=self.user, name='Vegan')
        tag2 = Tag.objects.create(user=self.user, name='Vegetarial')
        r1.tags.add(tag1)
        r2.tags.add(tag2)
    ...
    def test_filter_by_ingredients(self):
        ...
        r1 = create_recipe(user=self.user, title='First recipe')
        r2 = create_recipe(user=self.user, title='Second recipe')
        i1 = Ingredient.objects.create(user=self.user, name='Tomato')
...
class ImageUploadTest(TestCase):
    ...
    def test_upload_image_bad_request(self):
        """Test uploading a bad image to a recipe."""
        url = image_upload_url(self.recipe.id)
